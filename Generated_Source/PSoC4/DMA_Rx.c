...
uint8 DMA_Rx_initVar = 0u;


/*******************************************************************************
* Function Name: DMA_Rx_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == DMA_Rx_initVar)
    {
        DMA_Rx_Init();
        DMA_Rx_initVar = 1u;
    }
    CyDmaEnable();
    DMA_Rx_SetSrcAddress(0, srcAddress);
    DMA_Rx_SetDstAddress(0, dstAddress);
    DMA_Rx_ValidateDescriptor(0);
    DMA_Rx_ChEnable();
}
...
*  customizer. It is not necessary to call DMA_Rx_Init() because the
*  DMA_Rx_Start() API calls this function, which is the preferred method
...
{
    CYDMA_CH_CTL_BASE.ctl[DMA_Rx_CHANNEL] = DMA_Rx_CHANNEL_CFG;
    CYDMA_DESCR_BASE.descriptor[DMA_Rx_CHANNEL][0].ctl = DMA_Rx_DESCR0_CFG;
    CYDMA_DESCR_BASE.descriptor[DMA_Rx_CHANNEL][1].ctl = DMA_Rx_DESCR1_CFG;

    /* If the Generate IRQ option is selected for descriptor 0 or 1, set the
    * channel bit field in the DMA interrupt mask register.
    */
    #if (0u != ((DMA_Rx_DESCR0_CFG | DMA_Rx_DESCR1_CFG) & CYDMA_GENERATE_IRQ))
        CYDMA_INTR_MASK_REG |= DMA_Rx_CHANNEL_MASK;
    #endif /* (0u != ((DMA_Rx_DESCR0_CFG | DMA_Rx_DESCR1_CFG) & CYDMA_GENERATE_IRQ)) */
}
